...
private:
    ...
    double tarifOra_;          // cat cere pe ora (simplu, fara moneda acum)

public:
    Instructor(std::string nume, std::string email, std::string specializare, double tarifOra);
    ...
    // Operator membru (cerinta "alt operator supraincarcat ca functie membra")
    // Exemplu: marim tariful cu o suma
    Instructor& operator+=(double suma);
};

// Operator non-membru (cerinta "alt operator supraincarcat ca functie non-membra")
